...
- **Cloud (no PC required):** A GitHub Actions workflow runs daily at **midnight UAE time**. It fetches listings, runs `process_data.py`, then commits and pushes `data/dashboard_data.json`, a compressed SQLite history snapshot (`data/listings.db.gz`), and the fetched JSON files to `main`. Enable **Actions** in the repo and ensure the default branch is `main`; you can also trigger a run manually under **Actions → Listing Data Refresh → Run workflow**.
- **CI WAF bypass (required since Property Finder blocks GitHub datacenter IPs):** In **Settings → Secrets and variables → Actions**, add one of:
  - `PF_WAF_COOKIES` — open [propertyfinder.ae](https://www.propertyfinder.ae), complete any verification, then in DevTools → Network click a request to `propertyfinder.ae` → copy the full **Cookie** request header value (must include `aws-waf-token`). Refresh weekly or when runs fail.
  - `PF_PROXY_URL` — residential proxy URL, ideally UAE (e.g. `http://user:pass@host:port`).
- **DB retention and size guard:** The workflow keeps a rolling snapshot window in SQLite (`DB_RETENTION_SNAPSHOTS`, default `14`) before compressing. If compressed size is still too large, it automatically reduces retention down to `DB_MIN_RETENTION_SNAPSHOTS` (default `7`) and retries compression. Snapshot files are only committed if they are below the safe git threshold (`DB_GZ_MAX_BYTES`, default `95,000,000` bytes) to avoid GitHub's 100 MB hard limit.
- **If DB snapshot commit is skipped:** The run still commits `data/dashboard_data.json` and listing JSON files, and still uploads DB snapshots as workflow artifacts when present. This means dashboard updates continue even when DB snapshot files are too large for git.
